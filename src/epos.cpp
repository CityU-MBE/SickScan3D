...
// $Id: epos.cpp 388 2014-05-22 20:21:30Z zhangfy $
...
I have only checked the library to work with an EPOS 70/10 (firmware
...
/* *********************************************** */
/*    definitions used only internal in epos.cpp   */
/* *********************************************** */
...
//! Motor control (Profile Position Control) parameters define
int MaxProfVelocity; ///permitted maximum profile velocity
int ProfVelocity; ///profile velocity
int ProfAcceleration; ///profile acceleration
int ProfDeceleration; ///profile deceleration
...
int openEPOS(const char *dev) 
{
    ...
    */
    ...
    // open the rs232 terminal
    for(i=0;i<5;i++)
    {
	if((ep=open(dev,O_RDWR|O_NOCTTY|O_NDELAY))>=0)
	    break;
	sleep(1);
    }
...
int setOpMode(int m)
{
    ...
    {
	case E_PROFPOS:
	    return setProfile_Position_Mode(MaxProfVelocity, ProfVelocity, ProfAcceleration, ProfDeceleration);
	    break;
	default:
	    dw[1] = 0x0000; // high WORD of DWORD is not used here
	    dw[0] = m;
...
// set profile position mode and its parameters
int setProfile_Position_Mode(int max_velocity, int velocity, int acceleration, int deceleration)
{
    ...
    dw[0] = (WORD) (velocity & 0x0000FFFF);
    dw[1] = (WORD) (velocity >> 16) ;
    ...
    // store parameters
    dw[0] = 0x6173;
    dw[1] = 0x6576; // high WORD of DWORD is not used here
...
int readOpMode()
{
    ...
    */
    ...
    if (aa<0) 
	fprintf(stderr,"WARNING: EPOS is set to internal mode of operation (%hd).\n Make sure that this was really intended!\n", aa);
...
int monitorStatus()
{
	...
	printf("\rEPOS: pos=%+10ld |%+10ld (%ld to go); v= %+4ld | %+4ld[rpm]; I=%+4dmA", postarget, posactual,postarget-posactual, veldemand, velactual, curactual);
	fflush(stdout);
    ...
    printf("\r%d EPOS: pos=%+10ld |%+10ld (%ld to go); v= %+4ld | %+4ld[rpm]; I=%+4dmA\n", i, postarget, posactual, postarget-posactual, veldemand, velactual, curactual);
